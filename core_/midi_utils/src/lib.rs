//! midi_utils is a set of utilities to deal with midi messages and standard
//! midi files (SMF).  It handles both standard midi messages and the meta
//! messages that are found in SMFs.
//!
//! midi_utils is fairly low level, and  messages are stored and accessed in
...
pub use builder::{AbsoluteEvent, SMFBuilder};
pub use meta::{MetaCommand, MetaError, MetaEvent};
pub use midi::{make_status, MidiError, MidiMessage, Status, CHANNEL_MASK, STATUS_MASK};
use reader::SMFReader;
pub use util::note_num_to_name;
...
impl error::Error for SMFError {
	fn description(&self) -> &str {
		match *self {
			SMFError::InvalidSMFFile(_) => "Error SMFError::Error: The SMF file was invalid",
			SMFError::Error(ref other) => {
				eprintln!("Unknown Error: SMFError::Error: {}", other);
				"An Unknown SMFError::Error occured, check the stderr log for more information"
			},
			SMFError::MidiError(ref other) => {
				eprintln!("Unknown Error: SMFError::MidiError: {}", other);
				"An Unknown SMFError::MidiError occured, check the stderr log for more information"
			},
			SMFError::MetaError(ref other) => {
				eprintln!("Unknown Error: SMFError::MetaError: {}", other);
				"An Unknown SMFError::MetaError occured, check the stderr log for more information"
			},
		}
	}
...
impl fmt::Display for SMFError {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		match *self {
			...
			SMFError::MidiError(ref err) => {
				write!(f, "{}", err)
			},
			SMFError::MetaError(ref err) => {
				write!(f, "{}", err)
			},
