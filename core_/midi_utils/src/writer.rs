...
use crate::{AbsoluteEvent, Event, MetaCommand, MetaEvent, SMFFormat, SMF};
...
/// existing midi_utils::SMF.
///
/// # Writing an existing SMF to a file
/// ```rust
/// use midi_utils::{SMF,SMFWriter,SMFBuilder};
/// use std::path::Path;
/// // Create smf
...
impl SMFWriter {
	...
	pub fn vtime_to_vec(val: u64) -> Vec<u8> {
		...
		let cont_mask = 0x80_u8;
		let val_mask = 0x7F_u64;
		loop {
	...
	/// Add any sequence of AbsoluteEvents as a track to this writer.  A meta event with the given
	/// name will be added at the start of the track
