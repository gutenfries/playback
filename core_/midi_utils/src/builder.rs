use std::{cmp::Ordering, collections::BinaryHeap, ops::IndexMut};
...
impl TrackBuilder {
			...
			events: match self.events {
				EventContainer::Heap(heap) => {
					...
					for ev in absevents.into_iter() {
						let vtime = if prev_time == 0 { ev.time } else { ev.time - prev_time };
						prev_time = ev.time;
						events.push(TrackEvent { vtime, event: ev.event });
					}
					events
				},
	...
	fn abs_time_from_delta(&self, delta: u64) -> u64 {
		match self.events {
			...
			_ => {
				panic!("Can't call abs_time_from_delta on non-heap builder")
			},
		}
	}
}
...
/// ```rust
/// use midi_utils::SMFBuilder;
/// let mut builder = SMFBuilder::new();
/// builder.add_track();
/// ```
/// and then adding events to the track via
/// ```rust
/// use midi_utils::{Event, SMFBuilder, MetaEvent};
/// let mut builder = SMFBuilder::new();
/// let mut track = builder.add_track();
/// ```
/// and then adding
...
impl SMFBuilder {
	...
	where I: Iterator<Item=&'a AbsoluteEvent> {
	...
	pub fn add_midi_abs(&mut self, track: usize, time: u64, msg: MidiMessage) {
		...
		match self.tracks.index_mut(track).events {
			...
			_ => {
				panic!("Can't add events to static tracks")
			},
		}
	}
