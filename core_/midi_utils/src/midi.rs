...
impl error::Error for MidiError {
	fn description(&self) -> &str {
		match *self {
			...
			// Don't use the description() method for MidiError::Error
			// because it will just return "description() is deprecated"
			MidiError::Error(ref other) => {
...
impl MidiMessage {
	...
	pub fn next_message_running_status(
		stat: u8, databyte: u8, reader: &mut dyn Read,
	...
	/// greatest pressure value (of all the current depressed keys). `pressure` is the pressure
	/// value.
